...
const CardProtien = ({onClickHandler}) => {
  ...
  const proteinIdSuggestions = ["4V0Q", "4V0T", "4V1R", "4V0R", "4V02"];
  ...
  const onClickStartAnalysis = () => {
    if (proteinId1.trim() === '' || proteinId2.trim() === '') {
      ...
      setInputError(false); // Reset the input error if the input is not empty
       console.log(proteinId1, 'protein', proteinId2, selectedOption)
  ...
  // useEffect(() => {
    ...
    const fetchData = async (proteinId = 'able') => {
  ...
  {proteinData.map((suggestion, index) => (
    <div key={suggestion.id}>
      {suggestion.pdbs.map((item) => (
        ...
        // <option key={item.id} value={`${suggestion.name} - ${item.id}`} />
      ))}
    </div>
  ))}
</datalist>
      ...
      <label className= 'input_pro'>Second Protein Name</label>
        ...
        />
         ...
         <datalist id="proteinIdSuggestions2" className='proteinId_suggest'>
  {proteinData.map((suggestion, index) => (
    <div key={suggestion.id}>
      {suggestion.pdbs.map((item) => (
        <option key={item.id} value={item.id}>{suggestion.name} </option>
        // <option key={item.id} value={`${suggestion.name} - ${item.id}`} />
      ))}
    </div>
  ))}
